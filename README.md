...
Оператор `static_cast<>` используется для выполнения явного приведения типов во время компиляции. Он позволяет преобразовывать значения одного типа в другой, когда такое преобразование является безопасным и известным на этапе компиляции. Вот основные случаи использования `static_cast<>`
