...
В стандартной библиотеке реализован на deque(дэк). (`std::stack<T>`)
...
Можно реализовать через массив с проверко на дубликаты или хэш-функцию.

Основные операции, которые определяют интерфейс множества, включают:
* `insert` - вставка эллемента во множество.

В стандартной библиотеке представлены в виде `std::set<T>`.
