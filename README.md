...
<details>
	...
	<ol>
        ...
        <li><a href="#%D1%81%D1%82%D1%80%D0%BE%D0%BA%D0%B8-stdstring-%D0%BE%D0%B1%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%BA%D0%B0">Строки std::string, обработка</a></li>
...
ABI (Application Binary Interface) в контексте C++ определяет низкоуровневый интерфейс между различными модулями программы на уровне бинарных объектов. ABI включает в себя соглашения о вызовах функций, представление типов данных, выравнивание данных, правила передачи параметров и возврата значений, а также соглашения о компоновке классов, виртуальных функций и исключений.

Составляющие ABI:
* Соглашение о вызовах функций (Calling Convention):
Определяет, как параметры передаются в функцию и как возвращаются значения. Примеры включают `cdecl`, `stdcall`, `fastcall` и другие. В C++ это также касается передачи объектов и исключений.

* Выравнивание данных (Data Alignment):
Определяет, как данные выравниваются в памяти. Это включает выравнивание примитивных типов, таких как int и double, а также композитных типов, таких как структуры и классы.

* Компоновка объектов (Object Layout):
Определяет, как объекты классов компонуются в памяти, включая расположение данных-членов, выравнивание и порядок наследования. Также важно для понимания виртуальных таблиц (vtable) для классов с виртуальными функциями.

* Механизмы обработки исключений (Exception Handling):
Определяет, как исключения передаются и обрабатываются между различными объектными модулями, включая структуру таблиц исключений и стек вызовов.

* Имя манглирование (Name Mangling):
Компиляторы C++ используют имя манглирование для поддержки перегрузки функций и шаблонов. Это процесс преобразования имен функций и переменных в уникальные имена в объектных файлах.

<hr style="width:25%;text-align:left;margin-left:0">

## PIMPL

PIMPL (Pointer to IMPLementation) — это идиома проектирования в C++, которая используется для управления сложностью и обеспечения сокрытия деталей реализации класса от клиентского кода. Она позволяет изолировать интерфейс и детали реализации класса, минимизируя зависимости и упрощая изменения внутренней реализации без необходимости изменения интерфейса.

Одной из распространенных проблем в проектировании классов является необходимость раскрытия деталей реализации (implementation details) через заголовочные файлы. Это может включать в себя:
* Открытие данных-членов класса, которые клиентский код не должен знать.
* Открытие деталей реализации методов, которые необходимы только внутри класса.
* Ненужные зависимости от внутренних библиотек или сторонних заголовочных файлов.
* Решение с использованием PIMPL

Идиома PIMPL решает эти проблемы, скрывая детали реализации за указателем на имплементацию (implementation). В основе PIMPL лежит разделение класса на две части:
* Публичный интерфейс класса: Этот интерфейс определяется в заголовочном файле и содержит только методы и данные, доступные для клиентского кода.
* Приватная реализация: Детали реализации (implementation details) хранятся в отдельном классе или структуре, которая определяется в файле реализации (.cpp). Этот класс не экспортируется в заголовочный файл и доступен только внутри реализации класса.
...
private:
    MyClassImpl* pimpl; // Указатель на имплементацию
};
```
...
{
    // Доступ к приватным методам через pimpl
    pimpl->privateMethod();
}
```
...
RAII использует конструкторы для инициализации ресурсов и деструкторы для их автоматического освобождения при выходе из области видимости объекта. Это обеспечивает следующие преимущества:
* Гарантированное освобождение ресурсов: Ресурсы освобождаются независимо от того, как завершается блок кода — нормально или из-за исключения.
* Предотвращение утечек памяти: RAII помогает избежать утечек памяти, так как деструктор объекта автоматически вызывает функции или операторы освобождения ресурсов.
* Упрощение кода: Использование RAII снижает необходимость в явном вызове функций освобождения ресурсов и упрощает код благодаря автоматическому управлению ресурсами.
...
void useResource()
{
    Resource res; // Создание объекта Resource вызывает конструктор
    res.doSomething();
}   // Выход из блока вызывает деструктор, освобождающий ресурс
...
`std::shared_ptr` - класс, который содержит в себе указатель на массив или объект. На один и тот же массив или объект может указывать несколько `std::shared_ptr`. Когда на массив или объект указывает `std::shared_ptr`, создается блок управления, который считает количество `std::shared_ptr` и `std::weak_ptr`. Когда количество `std::shared_ptr` достигает 0, массив или объект уничтожается, а когда оба счетчика достигают 0, уничтожается и сам блок.

### Пример
```C++
int main()
{
    std::shared_ptr<int*> ptr1(new int[10]); // Создается массив из 10 int
    std::shared_ptr<int*> ptr2(ptr1); 
    ptr2.release();
    ptr1.release(); // Массив удаляется
    return 0;
}
```
...
`std::unique_ptr`- класс, который содержит в себе указатель на массив или объект. Данный массив или объект принадлежит только `unique_ptr`.

### Пример
```C++
int main()
{
    std::unique_ptr<float[]> ptr = std::make_unique(new float[50]);
}
```
...
`std::weak_ptr` - класс, который содержит в себе не-владеющую ссылку(указатель) на объект или массив, которым владеет `std::shared_ptr`. Получить доступ можно только конвертировав в `std::shared_ptr`.
...
int main()
{
    ...
    std::weak_ptr<int>::element_type val = *wp0.lock(); // Создает временный shared_ptr и разыменовывает его

    std::cout << "*wp0.lock() == " << val << std::endl; // Вывод: *wp0.lock() == 5;
    return 0;
}
```
...
Абстрактный класс - класс, который имеет хотя бы один чисто виртуальный метод. Абстрактный класс не имеет экземпляров. Можно получить указатель на абстрактный класс путем повышающего преобразования с производного класса(разыменования такого указателя ведет к неопределенному поведению). Производные классы тоже являются абстрактными, если они не переопределили все виртуальные функции.
...
Виртуальная функция - функция, которую предпологается переопределить в производном классе. Виртуальный вызов - вызов виртуальной функции, при вызове виртуальной функции вызывается функция, определенная в самом "нижнем" наследнике. Исключение: виртуальные функции в конструторе - там вызывается функция самого нижнего на данный момент собранного наследника.
...
Динамические полиморфизм - связывание интерфейса и реализации на этапе исполнения. Самый частый случай - вызов указателями на класс родителя виртуальных функций, определенных в классах-наследниках.

### vTable и vPtr
Когда в классе есть хотя бы один виртуальный метод, создается vTable. Каждой виртуальной функции соответствует 1 строка в таблице, которая содержит указатель на функцию в производном классе. vPtr - указатель на эту таблицу.
...
1. Конструирование производных объектов
...
2. Уничтожение производных объектов
